    ...
    if length != 0:
        for i in range(0, length):
            if lockTableObjects[i].lockedDateItem == resourceName:
                flag = 1
                if lockTableObjects[
                    i].lockState == WL:  # This checks if the resource taht's been requested in under a writeLock by any other transaction anfd if thats the case, wound wait is called
                    ...
                    i].lockState == RL:  # if resources is just under readLock, the new transaction is added to the list of transactions that hold a readLock on the resource
                    lockTableObjects[i].addLockHeld(transactionID);
                    searchTransactionID(transactionID).addLockedResource(resourceName)
                    print "Putting the data item " + resourceName + " under ReadLock by Transaction %d" % (
                        lockTableObjects[i].lockHeldBy[0])
        if flag == 0:
            ...
            if lockTableObjects[i].lockedDateItem == resourceName:
                ...
                if lockTableObjects[i].lockState == RL:
                    ...
                    woundWait(searchTransactionID(transactionID),
                              searchTransactionID(lockTableObjects[i].lockHeldBy[0]), lockTableObjects[i], inputLine)
        if flag == 0:
            lockTableObjects.append(
                lockTable(resourceName, transactionID, WL))  # appending a new resource to the lockTable
            searchTransactionID(transactionID).addLockedResource(resourceName)
                    ...
                    if resource.lockedDateItem == lock:
                        if len(resource.lockHeldBy) == 1:
                            lockTableObjects.remove(
                                resource)  # if the same sources is held by multiple transactions, remove this transaction from a that list of transactions holding the lock
                        else:
